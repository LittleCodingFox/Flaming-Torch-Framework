...
#	define TAG "UIManager"
	...
	void UIElement::OnEventPrivate(UIElement *Self, uint32 Type, std::vector<void *> Args)
	{
		...
		case UIEventType::MouseJustPressed:
			...
			EventErrorCache[Type] = !FailedArgCount && RunUIScriptEvents<UIElement *>(EventScriptHandlers[Type], NameValue, !EventErrorCache[Type], this, static_cast<const Input::MouseButtonInfo *>(Args[0]));

			if (PropagatesEvents() && !g_Input.InputConsumed() && ParentValue.Get())
		...
		case UIEventType::KeyJustPressed:
			...
			FailedArgCount = Args.size() != 1;
			EventErrorCache[Type] = !FailedArgCount && RunUIScriptEvents<UIElement *>(EventScriptHandlers[Type], NameValue, !EventErrorCache[Type], this, static_cast<const Input::KeyInfo *>(Args[0]));
		...
		case UIEventType::TouchJustPressed:
			...
			FailedArgCount = Args.size() != 1;
			EventErrorCache[Type] = !FailedArgCount && RunUIScriptEvents<UIElement *>(EventScriptHandlers[Type], NameValue, !EventErrorCache[Type], this, static_cast<const Input::TouchInfo *>(Args[0]));
		...
		case UIEventType::JoystickTap:
			...
			FailedArgCount = Args.size() != 1;
			EventErrorCache[Type] = !FailedArgCount && RunUIScriptEvents<UIElement *>(EventScriptHandlers[Type], NameValue, !EventErrorCache[Type], this, static_cast<const Input::JoystickButtonInfo *>(Args[0]));
		...
		case UIEventType::JoystickAxisMoved:
			...
			FailedArgCount = Args.size() != 1;
			EventErrorCache[Type] = !FailedArgCount && RunUIScriptEvents<UIElement *>(EventScriptHandlers[Type], NameValue, !EventErrorCache[Type], this, static_cast<const Input::JoystickAxisInfo *>(Args[0]));
	...
	void UIElement::Draw(const Vector2 &ParentPosition)
	{
		OnEvent(this, UIEventType::Draw, { const_cast<Vector2 *>(&ParentPosition) });
	}

	void UIElement::DrawUIFocusZone(const Vector2 &ParentPosition)
	{
		if(Manager()->DrawUIFocusZones)
		{
			...
			while(ParentPanel.Get())
			{
				if(ParentPanel->ParentPosition() == GlobalPosition)
					IndicatorPosition += RenderTextUtils::MeasureTextSimple(StringUtils::MakeIntString(ParentCounter), TextParams()).Right + 5;
	...
	void UIElement::DrawUIRect(const Vector2 &ParentPosition)
	{
