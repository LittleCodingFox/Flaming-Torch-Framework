...
#	define TAG2 "LuaScript"
	...
	void LuaErrorFunction(lua_State *State)
	{
		...
		if (g_LuaScript.ErrorStream)
			g_LuaScript.ErrorStream->Write2<char>(str.str().c_str(), str.str().length());

		g_Log.LogErr(TAG, str.str().c_str());
	}
	...
	void LuaEventGroup::Add(luabind::object Member)
	{
		if(!Member)
		{
			g_Log.LogWarn(TAG, "Unable to add a Member to a Lua Event Group: Invalid Member");
	...
	bool LuaLoadPackage(const std::string &Name)
	{
		...
		{
			g_Log.LogErr(TAG2, "Unable to add package '%s': Failed to open for reading", Name.c_str());

			return false;
		}

		if(!g_PackageManager.AddPackage(MakeStringID(Name), Stream))
		{
			g_Log.LogErr(TAG2, "Unable to add package '%s': Failed to deserialize", Name.c_str());

			return false;
		}

		g_Log.LogInfo(TAG2, "Loaded package '%s'", Name.c_str());
	...
	void LuaScriptManager::LogError(const std::string &Message)
	{
		...
		g_Log.LogErr(TAG, FinalMessage.c_str());
	}
	...
	void LuaScriptManager::StartUp(uint32 Priority)
	{
		...
		g_Log.LogInfo(TAG, "Lua ScriptManager Subsystem starting..");
		...
		if(!ErrorStream->Open(str.str(), StreamFlags::Write | StreamFlags::Text))
		{
			...
			g_Log.LogErr(TAG, "Lua ScriptManager running without ScriptErrors logging!");
		}
	...
	void LuaScriptManager::Shutdown(uint32 Priority)
	{
		...
		g_Log.LogInfo(TAG, "Lua ScriptManager Subsystem terminating..");
